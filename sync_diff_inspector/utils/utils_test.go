...
func TestGenerateSQLs(t *testing.T) {
	...
	// test value with "'", "\", "\n", "\r"
	rowsData["name"] = &dbutil.ColumnData{Data: []byte("a'a\\a\na\ra"), IsNull: false}
	replaceSQL = GenerateReplaceDML(rowsData, tableInfo, "diff_test")
	deleteSQL = GenerateDeleteDML(rowsData, tableInfo, "diff_test")
	require.Equal(t, replaceSQL, "REPLACE INTO `diff_test`.`atest`(`id`,`name`,`birthday`,`update_time`,`money`) VALUES (NULL,'a\\'a\\\\a\\na\\ra','2018-01-01 00:00:00','10:10:10',11.1111);")
	require.Equal(t, deleteSQL, "DELETE FROM `diff_test`.`atest` WHERE `id` is NULL AND `name` = 'a\\'a\\\\a\\na\\ra' AND `birthday` = '2018-01-01 00:00:00' AND `update_time` = '10:10:10' AND `money` = 11.1111 LIMIT 1;")
}
